...
| M4 | Projects and tasks support estimates, timers, sessions, subtasks and **one compact plain-text note each**; no Markdown or Obsidian workflow | Estimates/timers/sessions/subtasks by F1/F4. The note is **plain text** (migration 0011): the renderer is gone, `**bold**` renders as those nine characters, and the column is called `body` because a column called `markdown` is a standing invitation to put the parser back. "Compact" is 2000 characters, counted in characters rather than bytes so an Amharic note is not given a third of the room (`the_cap_counts_characters_rather_than_bytes`). Nothing anyone wrote was altered, and a note written under the old 1MB ceiling still opens — it can be cut down, it cannot be made longer (`a_note_written_before_the_cap_can_still_be_read_and_shortened`). A backup written before the rename still restores its notes (`a_backup_written_before_the_rename_still_restores_its_notes`). | ✅ |
...
| M9 | Manual life entries fill gaps, repeat, replace an interval with confirmation, and are editable later | `life_entries_fill_gaps_and_replace_with_confirmation`, plus **repeat** (migration 0012): the same engine the planner uses, because two recurrence implementations disagree about DST within a year and the one people notice is the one that moved their sleep. Instances are real rows, so any single night can be edited or removed on its own (`a_nightly_entry_becomes_real_rows_every_night`); unlike a block it may cross midnight, and the morning it covers still sees it (`a_repeating_night_is_visible_from_the_morning_it_covers`); a 23:00 bedtime stays at 23:00 across a DST boundary (`a_bedtime_keeps_its_wall_clock_across_a_dst_boundary`); and the scope of a deletion is asked, never inferred (`the_scope_of_a_deletion_is_asked_rather_than_inferred`). | ✅ |
...
| M12 | Reports open to a month summary by default; a month exports to `.xlsx` in the approved format with accurate totals | Reports is month-first; `get_month` is `get_day` summed, so a figure here and on a day cannot be computed two ways. The export carries **everything §4.8 enumerates**: the month matrix, daily and weekly totals, work by project/task and by contribution, life-area and sleep/rest target vs actual, and core / planned-entertainment / unplanned-entertainment / YouTube-Twitch. Totals are `COUNTIF` formulas over the sheet's own cells wherever the sheet can express the figure, and **each row states which it is** — a slot label cannot say whether an hour fell inside a plotted window, so those come from the record and say so. Formulas carry cached results, because a file whose numbers depend on which program opens it is not an export (`the_summary_carries_everything_the_spec_lists`, `every_figure_says_where_it_came_from`, `cached_results_are_what_the_formula_will_recompute`). | ✅ / format needs client sign-off |
...
**All 16 covered.** Two of them carry a caveat, and it is the same caveat both
