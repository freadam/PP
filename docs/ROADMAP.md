...
| **4. Day, Planner and dashboard** | Weeks 6–7 | **done** | The life-time model and precedence engine; the **Day view** at 5/15/30/60-minute resolution with empty hours, four distinguishable layers, a per-gap fill action and a day ledger. Planner at 1/3/7-day spans with drift, collisions, recurrence, and a month span. The month dashboard. Day-view editing beyond fill: **split, merge, repeat and multi-select**, plus per-project and per-area filters built from the day's own segments. | — |
| **5. Projects, tasks and life tracking** | Week 8 | **done** | Backlog with six groups, estimates on a fixed ladder, timers, manual session correction, subtasks to three levels. Life areas and entries, work contribution modes, and repeating life entries (migration 0012 — sleep is the case it exists for). Target-vs-actual bars for life areas on the month dashboard. Task notes reduced to compact plain text (migration 0011). | — |
| **6. Reconcile, calibrate, reduce entertainment** | Week 9 | **done** | Daily reconcile in the wireframe's three-column shape: queue, numbered choices with a stated recommendation, and an evidence panel for machine claims. Covers overruns, unstarted plans, unplanned work, **observed-only time and empty hours** (M10). Drift per block. Trailing 30-day calibration. Entertainment budgets (weekly goals, migration 0008) and **planned entertainment windows** (migration 0009), so planned-versus-unplanned reconciles to the intervals underneath it. Threshold notices, the off-plan nudge, fragmentation, focus sessions and the Monday-morning report all landed with [`PLAN-WEEKLY-GOALS.md`](PLAN-WEEKLY-GOALS.md). | — |
| **7. Excel migration and export** | Week 10 | **done, unproven** | The `.xlsx` month export: three sheets, a preview that is the sheet, options, and a reconciliation table putting the app's figures beside the sheet's own. Totals are formulas over the month sheet, not pasted numbers. JSON round-trips exactly; CSV exists. Workbook **import** with detection, a mapping nobody can skip, a signed per-day variance and an undo (M13, migration 0010). | The client's own workbook. It settles the export's format sign-off and gives the importer something real to be held against — open question 6, and a question for the client rather than for the build. |
...
6. ⬅ **The week you are in** — weekly goals with mid-week pacing,
   ...
   **W1–W8, W9's calibration and W10 are built** — migrations 0007 and 0008. Weekly goals
...
| Scope expands into notes, finance, mobile or blocking | Recorded as out of scope in `PRODUCT-SPEC.md` §1. The Markdown note renderer was a live instance and has been removed (migration 0011): plain text, 2000 characters, and the column renamed to `body` so the schema stops inviting the parser back. |
